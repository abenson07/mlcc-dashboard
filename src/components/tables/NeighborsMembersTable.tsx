...
export interface NeighborsMembersTableProps {
...
export default function NeighborsMembersTable({ onRowClick }: NeighborsMembersTableProps) {
  ...
  const newMembers = [...people]
    .filter((person) => isWithinLast30Days(person.membership?.created_at ?? null))
    .sort((a, b) => {
      const aDate = new Date(a.membership?.created_at ?? 0).getTime();
      const bDate = new Date(b.membership?.created_at ?? 0).getTime();
      return bDate - aDate;
    });
