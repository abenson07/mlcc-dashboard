...
export default function EventsListSidebar() {
  ...
  const onEventsList = pathname === "/old-admin/events";
  ...
  const sidebarGroups = [
    {
      ...
      items: councilEvents,
      hrefFor: (id: string) => `/old-admin/events-hub/${id}/overview` as const,
      isCouncil: true,
    },
    {
    ...
    },
    {
      ...
      hrefFor: (id: string) => `/old-admin/events?event=${id}` as const,
      isCouncil: false,
    },
  ] as const;
          ...
          <div key={group.label}>
            ...
            {group.items.map((event) => {
              ...
              const isActive = group.isCouncil
                ? pathname?.startsWith(`/old-admin/events-hub/${event.id}`)
