...
function groupTasks(tasks: EventTaskRow[], complete: boolean) {
  const filtered = tasks.filter((task) => task.isComplete === complete);
  const groups = new Map<EventTaskGroupLabel, EventTaskRow[]>();
  for (const task of filtered) {
    const list = groups.get(task.group) ?? [];
    list.push(task);
    groups.set(task.group, list);
  }
  const order = EVENT_TASK_GROUP_ORDER;
  ...
  return (
    ...
    >
      ...
      <span style={{ display: "flex", flexDirection: "column", flex: 1, minWidth: 0 }}>
        <Text size="sm" color={task.isComplete ? "secondary" : "primary"} style={task.isComplete ? { textDecoration: "line-through" } : undefined}>
          {task.title}
        </Text>
...
/**
 * Full-width Tasks view — active tasks grouped by "days out" from the
 * event, with per-group inline add, then a Completed section. Mirrors the
 * leaflet Schedule page.
 */
export function EventTasksPage({ tasks, onToggleTask, onAddTask, onRemoveTask }: EventTasksPageProps) {
