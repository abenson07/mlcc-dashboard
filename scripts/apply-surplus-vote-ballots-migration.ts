#!/usr/bin/env npx tsx
/**
 * Applies surplus_vote_ballots migration (DDL).
 * Usage: npx tsx scripts/apply-surplus-vote-ballots-migration.ts
 */
...
async function main(): Promise<void> {
  ...
  const migrationPath = path.join(
    REPO_ROOT,
    "supabase/migrations/20260917200000_surplus_vote_ballots.sql",
  );
  ...
  const { rows } = await client.query(
    "select to_regclass('public.surplus_vote_ballots') as tbl",
  );
