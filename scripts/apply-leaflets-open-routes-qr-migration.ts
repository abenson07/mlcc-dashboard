#!/usr/bin/env npx tsx
/**
 * Applies leaflets open-routes QR migration (DDL + backfill).
 * Usage: npx tsx scripts/apply-leaflets-open-routes-qr-migration.ts
 */
...
async function main(): Promise<void> {
  ...
  const migrationPath = path.join(
    REPO_ROOT,
    "supabase/migrations/20260703121000_leaflets_open_routes_qr.sql",
  );
  ...
  const { rows } = await client.query<{ column_name: string }>(
    `select column_name
     ...
     where table_schema = 'public'
       and table_name = 'leaflets'
       and column_name = 'open_routes_qr_code_id'`,
  );
