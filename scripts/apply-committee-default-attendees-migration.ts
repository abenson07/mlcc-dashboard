#!/usr/bin/env npx tsx
/**
 * Applies committee_default_attendees migration (DDL).
 * Usage: npx tsx scripts/apply-committee-default-attendees-migration.ts
 */
...
async function main(): Promise<void> {
  ...
  const migrationPath = path.join(
    REPO_ROOT,
    "supabase/migrations/20260624120000_committee_default_attendees.sql",
  );
  ...
  const { rows } = await client.query(
    "select to_regclass('public.committee_default_attendees') as tbl",
  );
