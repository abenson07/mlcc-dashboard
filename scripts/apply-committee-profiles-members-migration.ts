#!/usr/bin/env npx tsx
/**
 * Applies committee_profiles + committee_members migration (DDL).
 * Usage: npx tsx scripts/apply-committee-profiles-members-migration.ts
 */
...
async function main(): Promise<void> {
  ...
  const migrationPath = path.join(
    REPO_ROOT,
    "supabase/migrations/20260809140000_committee_profiles_and_members.sql",
  );
  ...
  const profiles = await client.query(
    "select to_regclass('public.committee_profiles') as tbl",
  );
  const members = await client.query(
    "select to_regclass('public.committee_members') as tbl",
  );
  await client.end();

  console.log("committee_profiles:", profiles.rows[0]?.tbl ?? null);
  console.log("committee_members:", members.rows[0]?.tbl ?? null);
}
